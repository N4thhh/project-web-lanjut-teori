<header class="bg-white shadow flex items-center justify-between px-8 py-2 relative z-20">
    @php
        ...
        $customerAvatar = $user && $user->avatar
            ? asset('storage/'.$user->avatar)
            : 'https://ui-avatars.com/api/?name='.urlencode($customerName).'&background=4FC3F7&color=fff';
    ...
    <div class="flex items-center space-x-3 lg:space-x-5">
        ...
        {{-- User + Logout --}}
        <div class="flex items-center space-x-3">
            {{-- FOTO + NAMA → LINK KE PROFIL --}}
            <a href="{{ route('customer.profile') }}" class="hidden sm:flex items-center space-x-2 group">
                <img
                    src="{{ $customerAvatar }}"
                    alt="{{ $customerName }}"
                    class="h-8 w-8 rounded-full object-cover border border-primary/20 group-hover:ring-2 group-hover:ring-primary/40 transition"
                >
                <span class="text-gray-700 font-medium text-sm truncate max-w-[140px] group-hover:text-primary">
                    {{ $customerName }}
                </span>
            </a>
